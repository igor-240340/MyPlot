...
namespace matplotlibcpp {
	namespace detail {
		...
		struct _interpreter {
			...
			PyObject* s_python_function_labellines;
			...
			static _interpreter* interkeeper(bool should_kill) {
				static _interpreter* ctx = NULL;

				if (ctx == NULL)
					ctx = new _interpreter;
...
#  endif
			...
			_interpreter() {
				...
				//std::cout << "Py_Initialize()\n";
				Py_Initialize();
				//std::cout << "matplotlibcpp.h:188\n";

				wchar_t const* dummy_args[] = { L"Python", NULL };  // const is needed because literals must not be modified
				wchar_t const** argv = dummy_args;
				int             argc = sizeof(dummy_args) / sizeof(dummy_args[0]) - 1;
				PySys_SetArgv(argc, const_cast<wchar_t**>(argv));
				std::cout << "matplotlibcpp.h:194\n";
				...
				PyObject* labellinesname = PyString_FromString("labellines");
				if (!pyplotname || !pylabname || !matplotlibname || !cmname || !labellinesname) {
					throw std::runtime_error("couldnt create string");
				}

				std::cout << "matplotlibcpp.h:208\n";
				...
				PyObject* labellinesmod = PyImport_Import(labellinesname);
				Py_DECREF(labellinesname);
				if (!labellinesmod) { throw std::runtime_error("Error loading module labellines!"); }
				...
				s_python_function_labellines = safe_import(labellinesmod, "labelLines");
#ifndef WITHOUT_NUMPY
			...
			~_interpreter() {
				OutputDebugString(L"[matplotlibcpp.h] ~_interpreter()\n");
				std::cout << "~_interpreter()\n";
				Py_Finalize();
			}
		};
	...
	bool plot(const std::vector<Numeric>& x, const std::vector<Numeric>& y, const std::map<std::string, std::string>& keywords)
	{
		...
		std::cout << "Py_IsInitialized():" << Py_IsInitialized() << std::endl;
		...
		if (PyErr_Occurred()) {
			...
			if (pvalue) {
				...
				if (pstr) {
					...
					std::cout << "error occured in plot: " << err_msg << std::endl;
				}
			}
		}
	...
	bool quiver(const std::vector<NumericX>& x, const std::vector<NumericY>& y, const std::vector<NumericU>& u, const std::vector<NumericW>& w, const std::map<std::string, std::string>& keywords = {})
	{
		...
		for (std::map<std::string, std::string>::const_iterator it = keywords.begin(); it != keywords.end(); ++it)
		{
			PyObject* item;
			if (it->first == "scale")
				item = PyFloat_FromDouble(std::strtod(it->second.c_str(), NULL));
			else
				item = PyUnicode_FromString(it->second.c_str());

			PyDict_SetItemString(kwargs, it->first.c_str(), item);
		}
	...
	{
		OutputDebugString(L"[matplotlib] subplot()\n");

		detail::_interpreter::get();

		OutputDebugString(L"[matplotlib] L:1940\n");
		...
		OutputDebugString(L"[matplotlib] L:1953\n");

		if (PyErr_Occurred()) {
			std::cout << "PyErr_Occurred()\n";
			//PyErr_PrintEx(0);
			//PyErr_Print();
	...
	inline void show(const bool block = true)
	{
		...
		if (PyErr_Occurred()) {
			...
			std::cout << "look: " << err_msg << std::endl;

			if (!pvalue)
				std::cout << "pvalue is null\n";

			const char* pStrErrorMessage = PyUnicode_AsUTF8(pvalue);
			std::cout << "look: " << pStrErrorMessage << std::endl;
	...
	inline void autoscale(const bool enable = true, const std::string& axis = "both", const bool tight = false)
	{
		std::cout << "autoscale()\n";
		detail::_interpreter::get();

		PyObject* pyEnable = enable ? Py_True : Py_False;
		Py_INCREF(pyEnable);

		PyObject* pyTight = tight ? Py_True : Py_False;
		Py_INCREF(pyTight);

		PyObject* args = PyTuple_New(3);
		PyTuple_SetItem(args, 0, pyEnable);
		PyTuple_SetItem(args, 1, PyString_FromString(axis.c_str()));
		PyTuple_SetItem(args, 2, pyTight);
		...
		/*if (enable && axis == "both" && tight == NULL)
		{
			std::cout << "2199\n";
			res = PyObject_CallObject(
				detail::_interpreter::get()->s_python_function_autoscale,
				detail::_interpreter::get()->s_python_empty_tuple);
			std::cout << "2203\n";
		}
		else
	...
	inline void labelLines()
	{
		...
		/*PyObject* axis = PyObject_CallObject(
			...
			detail::_interpreter::get()->s_python_empty_tuple);
		if (!axis) throw std::runtime_error("Call to gca() failed.");
		Py_INCREF(axis);*/

		/*PyObject* get_lines = PyObject_GetAttrString(axis, "get_lines");
		if (!get_lines) throw std::runtime_error("No get_lines");
		Py_INCREF(get_lines);*/

		/*PyObject* lines = PyObject_Call(
			get_lines,
			detail::_interpreter::get()->s_python_empty_tuple,
			detail::_interpreter::get()->s_python_empty_tuple);
		if (!lines) throw std::runtime_error("failed get_lines");
		Py_DECREF(axis);
		Py_DECREF(get_lines);*/
		...
		/*PyObject* args = PyTuple_New(1);
		PyTuple_SetItem(args, 0, lines);*/

		/*PyObject* kwargs = PyDict_New();
		PyDict_SetItemString(kwargs, "zorder", PyFloat_FromDouble(2.5));*/

		//PyObject* res = PyObject_Call(detail::_interpreter::get()->s_python_function_labellines, args, kwargs);
		PyObject* res = PyObject_Call(
			detail::_interpreter::get()->s_python_function_labellines,
			detail::_interpreter::get()->s_python_empty_tuple,
			detail::_interpreter::get()->s_python_empty_tuple);
		//Py_DECREF(args);
		//Py_DECREF(kwargs);
		if (!res) throw std::runtime_error("Call to labelLines() failed.");
		Py_DECREF(res);
	...
	inline void labelLastLine()
	{
		...
		int linesCount = PyList_Size(lines);
		std::cout << "linesCount:" << linesCount << std::endl;
		if (linesCount == 0) throw std::runtime_error("failed labelLastLine");
		PyObject* lastLine = PyList_GetItem(lines, linesCount - 1);
		PyObject* lastList = PyList_New(1);
		PyList_SetItem(lastList, 0, lastLine);

		PyObject* args = PyTuple_New(1);
		//PyTuple_SetItem(args, 0, lastList);
		PyTuple_SetItem(args, 0, lines);
